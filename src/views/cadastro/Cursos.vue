...
import ModalCadastroCursoVue from "./cursos/ModalCadastroCurso.vue";
import ModalEditarCursoVue from "./cursos/ModalEditarCurso.vue";
...
export default {
  ...
  methods: {
    async activeTipoCurso(item) {
      this.activeConfirm(item.nome, "tipos/cursos/" + item.id, async val => {
        if (val) {
          this.tableKey++;
        }
      });
    ...
    async desactiveTipoCurso(item) {
      this.deleteConfirm(item.nome, "tipos/cursos/" + item.id, async val => {
        if (val) {
          this.tableKey++;
        }
      });
